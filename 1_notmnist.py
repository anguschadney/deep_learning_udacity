...
test_dups = []
for idx, test in enumerate(hashes['test_hashes']):
    if test in hashes['train_hashes']:
            test_dups.append(idx)
len(test_dups)
...
valid_dups = []
for idx, valid in enumerate(hashes['valid_hashes']):
    if valid in hashes['train_hashes']:
            valid_dups.append(idx)
len(valid_dups)
...
test_valid_dups = []
for idx, test in enumerate(hashes['test_hashes']):
    if test in hashes['valid_hashes']:
            test_valid_dups.append(idx)
len(test_valid_dups)


# In[21]:

test_dups_unique = list(set(test_dups + test_valid_dups))
len(test_dups_unique)


# In[22]:

valid_dataset_unique = valid_dataset[~np.array(valid_dups)]
valid_labels_unique = valid_labels[~np.array(valid_dups)]
test_dataset_unique = test_dataset[~np.array(test_dups_unique)]
test_labels_unique = test_labels[~np.array(test_dups_unique)]
